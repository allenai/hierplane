...
class Toolbar extends React.Component {
  ...
  render() {
    ...
    if (disableJsonLink) {
      jsonBtn = (<div className="toolbar__button__link">JSON</div>);
    } else {
      if (serverEndPoint) {
        jsonBtn = (<a className="toolbar__button__link" target="_blank" href={`${window.API_URL}${jsonUrl}`}>JSON</a>);
      } else {
    ...
    return (
      <ul className="toolbar">
        {/* TODO: Add links for toggling expand/collape all, default/canonical layout toggle */}
        <li className={`toolbar__button ${disableJsonLink ? "toolbar__button--disabled" : ""}`}>
          {jsonBtn}
        </li>
      </ul>
    );
